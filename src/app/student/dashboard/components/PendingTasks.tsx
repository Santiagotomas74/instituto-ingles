...
import { Clock, RefreshCw, AlertCircle, Search, X } from "lucide-react";
  ...
  const loadTasks = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(t("tasks2.pending.error_title"));
        }

        const data = await res.json();

        if (!data.success) {
          throw new Error(data.message || t("tasks2.pending.error_title"));
        }
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : t("tasks2.pending.error_fallback");
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-5 rounded-2xl border border-gray-200/80 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              type="text"
              placeholder={t("tasks2.pending.search_placeholder")}
              value={search}
            ...
            {search && (
              <button
                ...
                aria-label={t("tasks2.pending.clear_search")}
              >
          ...
          <button
            ...
            disabled={refreshing}
            title={t("tasks2.pending.refresh")}
            aria-label={t("tasks2.pending.refresh")}
      ...
      {filteredTasks.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center space-y-3">
          ...
          <p className="text-sm font-medium text-gray-900">
            {t("tasks2.pending.no_tasks")}
          </p>

          <p className="text-xs text-gray-500">
            {t("tasks2.pending.no_tasks_description", { search })}
          </p>
