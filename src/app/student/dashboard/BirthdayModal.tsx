...
export default function BirthdayModal({ isBirthday }: { isBirthday: boolean }) {
  const [isOpen, setIsOpen] = useState(false);

  // Solo abrimos el modal si es el cumpleaños y no lo hemos cerrado aún
  useEffect(() => {
    if (isBirthday) {
      setIsOpen(true);
    }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-[32px] shadow-2xl max-w-sm w-full p-8 relative text-center animate-in fade-in zoom-in duration-300">
        ...
        <p className="text-slate-600 mb-8 leading-relaxed">
          ...
          <strong className="text-cyan-700">Instituto I.N.K</strong> te deseamos
          un excelente día. ¡Que se cumplan todos tus objetivos este año!
        </p>
